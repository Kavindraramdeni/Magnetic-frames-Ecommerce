...
interface CustomizerProps {
  ...
  onOpenCart?: () => void;
  appliedCoupon?: { code: string; label: string; discount: number } | null;
}
    ...
    try {
      ...
      if (orderRes.isMock || !orderRes.razorpayKeyId) {
        ...
        rzp.on('payment.failed', function (response: any) {
          ...
          setCheckoutError(`Payment error (${response.error.code}): ${response.error.description || 'Transaction declined.'}`);
                  ...
                  <div className="lg:col-span-7 space-y-4">
                    ...
                    {serviceabilityResult && (
                      serviceabilityResult.serviceable ? (
                        <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-xs flex items-center gap-2.5">
                          <Truck className="h-4 w-4 text-emerald-600 shrink-0" />
                          <div>
                            <p className="font-bold text-[11px] leading-tight">
                              ✅ Serviceable via {serviceabilityResult.courierName} ({serviceabilityResult.estimatedDays} Days)
                            </p>
                            <p className="text-[10px] text-emerald-700 font-sans mt-0.5">
                              Pincode {serviceabilityResult.pincode} • Delivery Charge: {serviceabilityResult.shippingCost === 0 ? 'FREE' : `₹${serviceabilityResult.shippingCost}`}
                            </p>
                          </div>
                        </div>
                      ) : (
                        <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-red-800 text-xs flex items-center gap-2.5">
                          <AlertCircle className="h-4 w-4 text-red-600 shrink-0" />
                          <div>
                            <p className="font-bold text-[11px] leading-tight">⚠️ Currently not available for this location</p>
                            <p className="text-[10px] text-red-600 font-sans mt-0.5">
                              {serviceabilityResult.error || `We do not deliver to pincode ${serviceabilityResult.pincode} yet.`}
                            </p>
                          </div>
                        </div>
                      )
